...
export function HistoryPage({ onVideoClick }: HistoryPageProps) {
  ...
  const loadHistory = async () => {
    ...
    if (data) {
      setViewHistory(data as ViewRecord[]);
    }
    setLoading(false);
  };
  ...
  const removeItem = async (viewId: string) => {
    const { error: deleteError } = await supabase
      .from('views')
      .delete()
      .eq('id', viewId);
    ...
    setViewHistory(viewHistory.filter(item => item.id !== viewId));
  };
      ...
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600">Your recently watched videos</p>
          </div>
        </div>
      ...
      {loading ? (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-500">
                  Watched {formatDistanceToNow(new Date(item.created_at))}
                </p>
              </div>
        ...
        <div className="flex flex-col items-center justify-center h-64 gap-4">
          ...
          <div className="text-center">
            <p className="text-gray-900 font-medium mb-1">No watch history</p>
